package com.dilatolyesi.web.rest;
...
import com.dilatolyesi.IntegrationTest;
import com.dilatolyesi.config.EmbeddedKafka;
...
class DilAtolyesiBackendKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dil-atolyesi-backend-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dil-atolyesi-backend-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/dil-atolyesi-backend-kafka/unregister"));
                return;
            }
        }
